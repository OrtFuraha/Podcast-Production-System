...
router.get('/:id', (req, res) => {
  ...
  db.get('SELECT * FROM episodes WHERE id = ?', [req.params.id], (err, row) => {
    ...
    db.run('UPDATE episodes SET views = views + 1 WHERE id = ?', [req.params.id]);
...
router.post('/', (req, res) => {
  const db = getDb(req);
  const { podcast_id, title, description, duration, audio_url, video_url, thumbnail, status, visibility } = req.body;
  ...
  db.run(
    ...
    [
      ...
      thumbnail || 'https://via.placeholder.com/400x160/000000/C1121F?text=Episode',
      status || 'draft',
      visibility || 'public'
    ],
    function(err) {
